...
/*Given a matrix a[0..n-1][0..n-1], this routine replaces it by the LU
 ...
 the partial pivoting; d is output as +/-1 depending on whether the number of
 row interchanges was even or odd, respectively. This routine is used in
 combination with solve to solve linear equations or invert a matrix.*/
...
void LU::solve(VecDoub_I &b, VecDoub_O &x) {
  ...
  for (i = 0; i < n; i++) {
    ...
    } else if (sum != 0.0) { // a nonzero element was encountered, do the sums
                             // in the loop above
      ii = i + 1;
    }
    x[i] = sum;
