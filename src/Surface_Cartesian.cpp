...
/****************************************************
 ...
 OUTPUT
 azimuth : azimuth angle at given coordinate
 *****************************************************/
/**
double Surface_Cartesian::Get_azimuth(Doub_I x, Doub_I y)
{
    double azimuth;
    double radi;
    ...
    if(abs(radi) < pow(10,-14))
    {
        cout << "When getting azimuth, point is too close to the origin" << endl;
        exit(0);
    }
    else if(y == 0)
    {
    ...
    {
        azimuth = 2*PI-acos(x/radi);
    }
    
    //Check
    if(azimuth >= 2*PI || azimuth < 0)
    {
        cout << "Azimuth angle not in the domain [0,2*PI)" << endl;
        exit(0);
    }
...
/****************************************************
 ...
 OUTPUT
 zenith : zenith angle at given point
 *****************************************************/
/**
double Surface_Cartesian::Get_zenith(Doub_I x, Doub_I y, Doub_I z)
{
    double zenith;
    double radi;
    
    radi = sqrt(x*x+y*y+z*z);
    
    if(abs(radi) < pow(10,-14))
    {
        cout << "When getting zenith, point is too close to the orgin" << endl;
        exit(0);
    }
    zenith = acos(z/radi);
    
    //Check
    if(zenith > PI || zenith < 0)
    {
        cout << "Zenith angle not in its domain [0,PI]" << endl;
        exit(0);
    }
...
/*******************************************************************************************************************
 Using a combination of Newton-Raphson and bisection, return the root of a function bracketed between x1 and x2.
 The root will be refined until its accuracy is known within xacc. funcd is a user-supplied struct that returns
 the function value as a functor and the first derivative of the function at the point x as the function df
 ...
 *******************************************************************************************************************/
double Surface_Cartesian::Rtsafe_x(const Doub_I x1, const Doub_I x2, const Doub_I y, const Doub_I z)
{
 ...
 *******************************************************************************************************************/
double Surface_Cartesian::Rtsafe_y(const Doub_I x, const Doub_I y1, const Doub_I y2, const Doub_I z)
{
    const int MAXIT = 100;                                            //Maximum allowed number of iterations
    double fl,fh,xh,xl,rts,dxold,dx,f,df,temp;
    int j;
 ...
 *******************************************************************************************************************/
double Surface_Cartesian::Rtsafe_z(const Doub_I x, const Doub_I y, const Doub_I z1, const Doub_I z2)
{
    const int MAXIT = 100;                                            //Maximum allowed number of iterations
    double fl,fh,xh,xl,rts,dxold,dx,f,df,temp;
    int j;
