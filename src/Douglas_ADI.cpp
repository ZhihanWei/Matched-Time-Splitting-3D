        ...
        for(int iz = 1; iz < nz-1; iz++)
        {
            ...
            while(ip < inter.ifpy[ix][iz].size())
            {
                ...
                if(inter.ifpy[ix][iz][ip].ID > 0)
                {
                    ...
                    for(int i = 0; i < 4; i++)
                    {
                        sum += vdey[2]*uh[ix][data.left_loc-1+i][iz]*data.wei.weir[0][i];
                    }
                    uhs[ix][iy][iz] = sum*dt;
                    ...
                    for(int i = 0; i < 4; i++)
                    {
                        sum += vdey[0]*uh[ix][data.left_loc-1+i][iz]*data.wei.weil[0][i];
                    }
                    uhs[ix][iy][iz] = sum*dt;
                ...
                else
                {
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdey[2]*uh[ix][datal.left_loc-1+i][iz]*datal.wei.weir[0][i];
                    }
                    uhs[ix][iy][iz] = sum*dt;
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdey[0]*uh[ix][datal.left_loc-1+i][iz]*datal.wei.weil[0][i];
                    }
                    //IY,  no FP
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdey[2]*uh[ix][datar.left_loc-2+i][iz]*datar.wei.weir[0][i];
                    }
                    uhs[ix][iy][iz] = sum*dt;
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdey[0]*uh[ix][datar.left_loc-2+i][iz]*datar.wei.weil[0][i];
                    }
                    uhs[ix][iy][iz] = sum*dt;
        ...
        for(int iy = 1; iy < ny-1; iy++)
        {
            ...
            while(ip < inter.ifpz[ix][iy].size())
            {
                ...
                if(inter.ifpz[ix][iy][ip].ID > 0)
                {
                    ...
                    for(int i = 0; i < 4; i++)
                    {
                        sum += vdez[2]*uh[ix][iy][data.left_loc-1+i]*data.wei.weir[0][i];
                    }
                    ...
                    for(int i = 0; i < 4; i++)
                    {
                        sum += vdez[0]*uh[ix][iy][data.left_loc-1+i]*data.wei.weil[0][i];
                    }
                    uhs2[ix][iy][iz] = sum*dt;
                ...
                else
                {
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdez[2]*uh[ix][iy][datal.left_loc-1+i]*datal.wei.weir[0][i];
                    }
                    uhs2[ix][iy][iz] = sum*dt;
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdez[0]*uh[ix][iy][datal.left_loc-1+i]*datal.wei.weil[0][i];
                    }
                    //IZ,  no FP
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdez[2]*uh[ix][iy][datar.left_loc-2+i]*datar.wei.weir[0][i];
                    }
                    uhs2[ix][iy][iz] = sum*dt;
                    ...
                    for(int i = 0; i < 5; i++)
                    {
                        sum += vdez[0]*uh[ix][iy][datar.left_loc-2+i]*datar.wei.weil[0][i];
                    }
                    uhs2[ix][iy][iz] = sum*dt;
 ...
 ********************************************************************************/
void Douglas_ADI::Src_2nd(Equation& eq, Intersections& inter, Beta& beta, CubicDoub_I& uh)
{
    int indx, ip;
    int ix, iy, iz;
    VecDoub vdex, vdey, vdez;
        ...
        for(int iz = 1; iz < nz-1; iz++)
        {
            ...
            while(ip < inter.ifpx[iy][iz].size())
            {
                ...
                if(inter.ifpx[iy][iz][ip].ID > 0)
                {
                    ...
                    src[ix][iy][iz] += vdex[2]*(jump_u*data.wei.weir[0][4]+jump_betaux*data.wei.weir[0][5]);
                    
                    //Approximate IX+1
                    ix += 1;
                    Operator_weights_x(beta,vdex,ix,iy,iz,dx);
                    //IX-1 cross interface, use left FP
                    src[ix][iy][iz] += vdex[0]*(jump_u*data.wei.weil[0][4]+jump_betaux*data.wei.weil[0][5]);
                ...
                else
                {
                    ...
                    src[ix][iy][iz] += vdex[2]*(jump_ul*datal.wei.weir[0][5]+jump_betauxl*datal.wei.weir[0][6]+
                    ...
                    Operator_weights_x(beta,vdex,ix,iy,iz,dx);
                    //IX-1 cross left interface, use left interface's left FP
                    src[ix][iy][iz] += vdex[0]*(jump_ul*datal.wei.weil[0][5]+jump_betauxl*datal.wei.weil[0][6]+
                                                jump_ur*datal.wei.weil[0][7]+jump_betauxr*datal.wei.weil[0][8]);
                    //IX+1 cross right interface, use right interface's right FP
                    src[ix][iy][iz] += vdex[2]*(jump_ul*datar.wei.weir[0][5]+jump_betauxl*datar.wei.weir[0][6]+
                    ...
                    src[ix][iy][iz] += vdex[0]*(jump_ul*datar.wei.weil[0][5]+jump_betauxl*datar.wei.weil[0][6]+
        ...
        for(int iz = 1; iz < nz-1; iz++)
        {
            ...
            while(ip < inter.ifpy[ix][iz].size())
            {
                ...
                if(inter.ifpy[ix][iz][ip].ID > 0)
                {
                    ...
                    Operator_weights_y(beta,vdey,ix,iy,iz,dy);
                    //IY+1 cross interface, use right FP
                    src[ix][iy][iz] += vdey[2]*(jump_u*data.wei.weir[0][4]+jump_betauy*data.wei.weir[0][5]);
                    
                    //Approximate IY+1
                    iy += 1;
                    Operator_weights_y(beta,vdey,ix,iy,iz,dy);
                    //IY-1 cross interface, use left FP
                    src[ix][iy][iz] += vdey[0]*(jump_u*data.wei.weil[0][4]+jump_betauy*data.wei.weil[0][5]);
                ...
                else
                {
                    ...
                    Operator_weights_y(beta,vdey,ix,iy,iz,dy);
                    //IY+1 cross left interface, use middle FP
                    src[ix][iy][iz] += vdey[2]*(jump_ul*datal.wei.weir[0][5]+jump_betauyl*datal.wei.weir[0][6]+
                    ...
                    Operator_weights_y(beta,vdey,ix,iy,iz,dy);
                    //IY-1 cross left interface, use left interface's left FP
                    src[ix][iy][iz] += vdey[0]*(jump_ul*datal.wei.weil[0][5]+jump_betauyl*datal.wei.weil[0][6]+
                                                jump_ur*datal.wei.weil[0][7]+jump_betauyr*datal.wei.weil[0][8]);
                    //IY+1 cross right interface, use right interface's right FP
                    src[ix][iy][iz] += vdey[2]*(jump_ul*datar.wei.weir[0][5]+jump_betauyl*datar.wei.weir[0][6]+
                    ...
                    Operator_weights_y(beta,vdey,ix,iy,iz,dy);
                    //IY-1 cross right interface, use middle FP
                    src[ix][iy][iz] += vdey[0]*(jump_ul*datar.wei.weil[0][5]+jump_betauyl*datar.wei.weil[0][6]+
        ...
        for(int iy = 1; iy < ny-1; iy++)
        {
            ...
            while(ip < inter.ifpz[ix][iy].size())
            {
                ...
                if(inter.ifpz[ix][iy][ip].ID > 0)
                {
                    ...
                    Operator_weights_z(beta,vdez,ix,iy,iz,dz);
                    //IZ+1 cross interface, use right FP
                    src[ix][iy][iz] += vdez[2]*(jump_u*data.wei.weir[0][4]+jump_betauz*data.wei.weir[0][5]);
                    
                    //Approximate IZ+1
                    iz += 1;
                    Operator_weights_z(beta,vdez,ix,iy,iz,dz);
                    //IZ-1 cross interface, use left FP
                    src[ix][iy][iz] += vdez[0]*(jump_u*data.wei.weil[0][4]+jump_betauz*data.wei.weil[0][5]);
                ...
                else
                {
                    ...
                    Operator_weights_z(beta,vdez,ix,iy,iz,dz);
                    //IZ+1 cross left interface, use middle FP
                    src[ix][iy][iz] += vdez[2]*(jump_ul*datal.wei.weir[0][5]+jump_betauzl*datal.wei.weir[0][6]+
                    ...
                    Operator_weights_z(beta,vdez,ix,iy,iz,dz);
                    //IZ-1 cross left interface, use left interface's left FP
                    src[ix][iy][iz] += vdez[0]*(jump_ul*datal.wei.weil[0][5]+jump_betauzl*datal.wei.weil[0][6]+
                                                jump_ur*datal.wei.weil[0][7]+jump_betauzr*datal.wei.weil[0][8]);
                    //IZ+1 cross right interface, use right interface's right FP
                    src[ix][iy][iz] += vdez[2]*(jump_ul*datar.wei.weir[0][5]+jump_betauzl*datar.wei.weir[0][6]+
                    ...
                    Operator_weights_z(beta,vdez,ix,iy,iz,dz);
                    //IZ-1 cross right interface, use middle FP
                    src[ix][iy][iz] += vdez[0]*(jump_ul*datar.wei.weil[0][5]+jump_betauzl*datar.wei.weil[0][6]+
...
/**********************************************************************************************************
 Calculate weights with variable diffusion coefficients for 2nd order, beta*Delta_{xx}
 ...
 **********************************************************************************************************/
void Douglas_ADI::Operator_weights_x(Beta& beta, VecDoub_O& vdex, Int_I ix, Int_I iy, Int_I iz, Doub_I dx)
{
    int indx;
    double beta1, beta2;
...
/**********************************************************************************************************
 Calculate weights with variable diffusion coefficients for 2nd order, beta*Delta_{yy}
 ...
 **********************************************************************************************************/
void Douglas_ADI::Operator_weights_y(Beta& beta, VecDoub_O& vdey, Int_I ix, Int_I iy, Int_I iz, Doub_I dy)
{
    int indx;
    double beta1, beta2;
...
/**********************************************************************************************************
 Calculate weights with variable diffusion coefficients for 2nd order, beta*Delta_{zz}
 ...
 **********************************************************************************************************/
void Douglas_ADI::Operator_weights_z(Beta& beta, VecDoub_O& vdez, Int_I ix, Int_I iy, Int_I iz, Doub_I dz)
{
    int indx;
    double beta1, beta2;
