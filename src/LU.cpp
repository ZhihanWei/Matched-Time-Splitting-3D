...
/*Given a matrix a[0..n-1][0..n-1], this routine replaces it by the LU decomposition of a
 rowwise permutation of itself. a is input. On output, indx[0..n-1] is an output vector
...
LU::LU(MatrixDoub_I &a): n((int)a.size()),lu(a),aref(a),indx(n)
{
    ...
    for (k = 0; k < n; k++)                        //this is the outermost kth loop
    {
...
/*Solves the set of n linear equations Ax=b using the stored LU decomposition of A.
...
/*Solves m sets of n linear equations AX=B using the stored LU decomposition of A.
 ...
 returns the solution A-1*B. b and x may reference the same matrix, in which case
...
void LU::solve(MatrixDoub_I &b, MatrixDoub_O &x)
{
    ...
    m = (int)b[0].size();
    if ((int)b.size() != n || (int)x.size() != n || (int)b[0].size() != (int)x[0].size())
    {
